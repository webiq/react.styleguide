...
After installing this package you can add this config to the _extends_ array:
...
// wrong
const result = numbers.map((n) => n * 2).reduce((sum, cur) => sum + cur, 0);
...
const HelloWorld: React.FC = () => <Text>Hello, world</Text>;
```
...
const Hello: React.FC<HelloProps> = (props) => <Text>Hello, {props.name}</Text>;
```
